...
app.mount("/static", StaticFiles(directory="posture_data"), name="static")
...
def predict_posture(features, models):
    ...
    with tf.device('/CPU:0'):
        ...
        return None, 0.0
def camera_thread():
    global camera_active, last_prediction, current_frame
    global bad_posture_tracking

    # Giảm số lần thử và thời gian chờ giữa các lần thử
    max_attempts = 2
    for attempt in range(max_attempts):
        cap = cv2.VideoCapture(0)
        if cap.isOpened():
            break
        print(f"Lần thử mở camera thứ {attempt+1} thất bại, thử lại...")
        time.sleep(0.2)  # Giảm thời gian chờ từ 1s xuống 0.2s
    
    if not cap.isOpened():
        print("Không thể mở camera sau nhiều lần thử")
        camera_active = False
        return

    # Giảm độ phân giải để tăng tốc xử lý
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 480)  # Giảm từ 640 xuống 480
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 360)  # Giảm từ 480 xuống 360
    cap.set(cv2.CAP_PROP_FPS, 15)  # Giảm từ 30 xuống 15 để giảm tải

    frame_count = 0
    prediction_interval = 15  # Tăng từ 10 lên 15 để giảm tần suất dự đoán
    ...
    while camera_active:
        ...
        if frame_count % prediction_interval == 0:
            # Giảm kích thước frame trước khi xử lý để tăng tốc
        ...
        # Đưa frame vào queue - sử dụng cơ chế thay thế frame cũ
        try:
            # Xóa frame cũ nếu có
...
async def get_audio(posture: str):
    if posture in POSTURE_AUDIO_MAPPING:
        audio_file = POSTURE_AUDIO_MAPPING[posture]
        audio_url = f"/static/audio_alerts/{audio_file}"
        return JSONResponse({
            "url": audio_url,
            "filename": audio_file
        })
...
async def check_posture_alert():
    ...
    if current_bad_posture and current_bad_posture not in ["straight_back", "vai_thang"]:
        ...
        if duration >= 5 and (current_time - last_alert_time >= ALERT_COOLDOWN):
            ...
            audio_file = POSTURE_AUDIO_MAPPING.get(current_bad_posture, "")
            return JSONResponse({
                "alert": True, 
...
async def start_camera():
    ...
    if not camera_active:
        camera_active = True
        threading.Thread(target=camera_thread, daemon=True).start()
        # Đợi một chút để camera có thể khởi động
...
async def get_snapshot():
    """Trả về một ảnh snapshot từ camera."""
    if not camera_active:
        return JSONResponse(
            status_code=400,
            content={"message": "Camera không hoạt động"}
        )
    ...
    with frame_lock:
        if current_frame is None:
            return JSONResponse(
                status_code=404,
                content={"message": "Không có frame nào từ camera"}
            )
        ...
        if not ret:
            return JSONResponse(
                status_code=500,
                content={"message": "Lỗi khi mã hóa hình ảnh"}
            )
        
        # Trả về hình ảnh như một phản hồi
